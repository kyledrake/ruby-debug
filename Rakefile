...
namespace :base do
  ...
  desc "Write #{base_spec.spec_name}"
  task :spec do
    File.open(base_spec.spec_name, 'w'){|f| f.write(base_spec.to_ruby)}
  end
end
...
namespace :cli do
  ...
  desc "Write #{cli_spec.spec_name}"
  task :spec do
    File.open(cli_spec.spec_name, 'w'){|f| f.write(cli_spec.to_ruby)}
  end
end
